...
/* loading: 
   the question is - how many different ways can we load? 
   
   iframe 
   script - can only be used for scripts 
   link - can only be used for css (possibly other things too but not sure how it works) 
   
   ajax - this is the most general solution - so we will start there... will try to 
   have an API similar to jQuery... 
...
(function () {
	...
	// pretty much all commands can be done via selection!!!... 
	, selection : function () {
	    this.focus(); 
	...
	, setupHooks : function () {
	    var me = this; 
	    bzj.debug.log('richtext setup hooks: ', me.doc.readyState); // so why don't it bind?? 
	    bzj.debug.log('richtext - type of me.doc: ', bzj.node.isa(me.doc)); // hmm..!!! 
	    // alert(me.doc); 
	    bzj.$(me.doc).hotkey('ctrl-b', function (evt) {
		me.exec('bold'); 
		return false; 
	    }); 
	    
	    bzj.$(me.doc).hotkey('ctrl-i', function (evt) {
		me.exec('italic'); 
		return false; 
	    }); 
	    
	    bzj.$(me.doc).hotkey('ctrl-k', function (evt) {
		me.html('<b>test me out</b>', true); 
		// great it does not fail.. 
		alert('text: ' + bzj.node.text(bzj.node.make('hr'))); 
	    }); 
	    
	    bzj.$(me.doc).hotkey('ctrl-h', function (evt) {
		var start = me.selection().range().start(); // start will require a different thing to do... 
		alert('bold? ' + EasyBookmark.insideBold(start.node) + '\n'
		      + 'italic? ' + EasyBookmark.insideItalic(start.node) + '\n'
		      + 'node: ' + bzj.node.toPath(start.node) + '\n' 
		      + 'is a Node: ' + bzj.node.isa(start.node) + ' and ' + bzj.node.isa(start) + '\n' 
		      + me.value()); //*/
		return false; 
	    }); 
	    
	    bzj.$(me.doc).hotkey('ctrl-s', function (evt) {
		bzj.debug.log('capture undo'); 
		me.exec('capture'); 
		return false; 
	    });
	    
	    bzj.$(me.doc).hotkey('ctrl-z', function (evt) {
		me.exec('undo'); 
		return false; 
	    }); 
	    
	    bzj.$(me.doc).hotkey('ctrl-y', function (evt) {
		me.exec('redo'); 
		return false; 
	    }); 
	    
	    /* setup all of the hooks for the button class in the page.. */ 
	    bzj.$('.button', me.editor).click (function (evt) {
		// this's name... 
		me.exec(this.name);
		return false;
	    }); 
	    
	    bzj.debug.log('richtext setup hooks done'); 
	}
     ...
     * 
     * 3 - command pattern.  each command should have access to a set of necessary public API.
     *     they have access to the internal API but that might not be supported. 
     * 
     * 4 - toolbar/buttons.  this needs to be integrated with the image script, since the buttons 
     *     
     *     are loaded there - and we need to ensure that the images can be *shipped* with the editor 
     *     itself eventually... 
     *
    ...
    // how do we trigger each command? 
    // a single command can have the following:
    // a callback. 
    // a set of hotkey... or a set of button/menus... 
    // honestly... the button/menu have almost nothing to do with the shortcuts... 
